...
export default async function handler(
  ...
  if (req.method === "POST") {
    const { amount, orderId, customerDetails, formData, savedFormData } =
      req.body;
    const fullAddress = `${savedFormData?.address}, ${savedFormData?.village}, ${savedFormData?.district}, ${savedFormData?.regency}, ${savedFormData?.province}`;

    const core = new MidtransClient.CoreApi({
      isProduction: false,
      serverKey: Buffer.from(
        `${process.env.NEXT_PRIVATE_MIDTRANS_SERVER_KEY}:`
      ),
      clientKey: Buffer.from(
        `${process.env.NEXT_PRIVATE_MIDTRANS_CLIENT_KEY}:`
      ),
    });
    ...
    try {
      ...
      if (chargeResponse && chargeResponse.actions) {
        const qrisAction = chargeResponse.actions.find(
          (action: { name: string }) =>
            action.name === "generate-qr-code" || action.name === "qris"
        );
        ...
        if (qrisAction) {
          ...
          res
            .status(500)
            .json({ message: "QR Code tidak ditemukan dalam response." });
        }
      } else {
        res.status(500).json({
          message: "Tidak ada tindakan yang valid dalam response transaksi.",
        });
      }
    } catch (error: any) {
    ...
    try {
      const transactionStatus = await core.transaction.status(
        orderId as string
      );
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Gagal membatalkan transaksi", error: error.message });
    }
  } else {
